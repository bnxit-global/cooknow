...
export default function AIInputDeck({ ingredients, onAdd, onRemove }: AIInputDeckProps) {
  ...
  const handleCommand = (cmd: string) => {
    ...
    setTimeout(() => {
      // Direct Match
      const match = allIngredients.find(ing => ing.toLowerCase() === cmd.toLowerCase());

      // Partial Match
      const partial = allIngredients.find(ing => cmd.toLowerCase().includes(ing.toLowerCase()));
    ...
    <div className="flex flex-col h-full p-8 relative">
      ...
      <div className="mb-8 p-4 bg-white/5 border border-white/10 rounded-lg backdrop-blur-md">
        <div className="flex justify-between items-center mb-2">
          <span className="text-[10px] uppercase font-bold tracking-widest text-white/40">Status Monitor</span>
      ...
      <div className="relative mb-8 z-50" ref={dropdownRef}>
        ...
        {suggestions.length > 0 && (
          <div className="absolute top-full left-0 right-0 mt-2 bg-[#0c0a09] border border-white/10 rounded-xl overflow-hidden shadow-2xl z-50">
            {suggestions.map((s, i) => (
              <button
                key={i}
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar">
        <h3 className="text-[10px] uppercase font-bold tracking-widest text-white/30 mb-4 flex items-center gap-2">
          Active Inventory
          <span className="bg-white/10 px-1.5 py-0.5 rounded text-white text-[9px]">{ingredients.length}</span>
        </h3>
        ...
        {ingredients.length > 0 ? (
            ...
            {ingredients.map(ing => (
              ...
              >
                <div className="flex justify-between items-start">
                  <span className="text-sm font-medium text-white/90 capitalize">{ing}</span>
                  <button
          ...
          <div className="h-32 flex flex-col items-center justify-center border border-dashed border-white/10 rounded-lg bg-white/[0.01]">
            <span className="text-white/20 text-xs font-mono">NO DATA LOGGED</span>
          </div>
        )}
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-white/5 flex justify-between items-center text-[10px] text-white/20 font-mono">
        ...
        <span className={isProcessing ? "text-blue-500 animate-pulse" : "text-green-500"}>
